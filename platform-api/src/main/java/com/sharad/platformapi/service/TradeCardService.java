...
public class TradeCardService {
        ...
        public TradeCardDto generate(
                ...
                String signal = signalStrengthService.generateSignals()
                                .stream()
                                .filter(
                ...
                MarketRegime regime = marketRegimeRepository.findLatestRegime();
                ...
                ETFPriceHistory latest = repository.findTopBySymbolOrderByTradeDateDesc(
                                symbol);

                List<ETFPriceHistory> candles = repository.findTop20BySymbolOrderByTradeDateDesc(
                                symbol);

                BigDecimal entryPrice = latest.getClosePrice();

                BigDecimal atr = calculateAtr(
                                candles);

                BigDecimal swingLow = candles.stream()
                                .map(
                ...
                BigDecimal stopLoss = atrStop.max(
                                swingLow);

                BigDecimal atrMultiplier = BigDecimal.valueOf(3);

                BigDecimal atrTarget = entryPrice.add(
                                atr.multiply(
                                                atrMultiplier));

                BigDecimal pivotResistance = findPivotResistance(
                                candles,
                                entryPrice);
                ...
                if (pivotResistance != null
                                &&
                                pivotResistance.compareTo(
                                                atrTarget) > 0) {

                        targetPrice = pivotResistance;

                        targetSource = "PIVOT_RESISTANCE";

                } else {

                        targetPrice = atrTarget;

                        targetSource = "ATR_3X";
                }

                BigDecimal expectedReturn = targetPrice.subtract(
                                entryPrice)
                                .divide(
                                                entryPrice,
                                                4,
                ...
                BigDecimal expectedRisk = entryPrice.subtract(
                                stopLoss)
                                .divide(
                                                entryPrice,
                                                4,
                ...
                BigDecimal riskReward = expectedReturn.divide(
                                expectedRisk,
                                2,
                                RoundingMode.HALF_UP);

                BigDecimal trailingStop = entryPrice.multiply(
                                BigDecimal.valueOf(
                                                0.92))
                                .setScale(
                                                2,
                ...
                String targetConfidence = determineTargetConfidence(
                                signal,
                                targetSource);

                int tradeQualityScore = calculateTradeQualityScore(
                                signal,
                                riskReward);

                String tradeGrade = determineTradeGrade(
                                tradeQualityScore);
        ...
        private BigDecimal findPivotResistance(
                ...
                for (int i = 2; i < candles.size() - 2; i++) {
                        ...
                        BigDecimal current = candles.get(i)
                                        .getHighPrice();

                        boolean pivot = current.compareTo(
                                        candles.get(i - 1)
                                                        .getHighPrice()) > 0
                                        &&
                                        current.compareTo(
                        ...
                        if (pivot
                                ...
                                BigDecimal distance = current.subtract(
                                                entryPrice);

                                BigDecimal atrThreshold = entryPrice.multiply(
                                                BigDecimal.valueOf(
                                                                0.01));
        ...
        private String determineTargetConfidence(
                ...
                if ("PIVOT_RESISTANCE".equals(source)
                                &&
                                "STRONG_BUY".equals(signal)) {
                        return "HIGH";
                }
        ...
        private int calculateTradeQualityScore(
                ...
                int score = riskReward.multiply(
                                BigDecimal.valueOf(20))
                                .intValue();

                if ("STRONG_BUY".equals(signal)) {
        ...
        private BigDecimal calculateAtr(
                ...
                for (int i = 1; i < Math.min(
                        ...
                        ETFPriceHistory current = candles.get(i - 1);

                        ETFPriceHistory previous = candles.get(i);

                        BigDecimal tr1 = current.getHighPrice()
                                        .subtract(
                                                        current.getLowPrice());

                        BigDecimal tr2 = current.getHighPrice()
                                        .subtract(
                                                        previous.getClosePrice())
                                        .abs();

                        BigDecimal tr3 = current.getLowPrice()
                                        .subtract(
                                                        previous.getClosePrice())
                                        .abs();

                        BigDecimal tr = tr1.max(tr2)
                                        .max(tr3);
